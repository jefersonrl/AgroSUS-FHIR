Componente 5 do DAK — *Elementos de dados centrais*. O AgroSUS já possui uma camada de
terminologia extensa: 17 `CodeSystem` e 9 `ValueSet` locais, além de vocabulários padrão (LOINC
para exames/observações, CID-10 para diagnósticos, CBO para ocupações). Ver o
<a href="artifacts.html">Artifact Index</a> para a lista completa.

> **Codificação.** Vocabulários padrão são usados sempre que existem (LOINC, CID-10, CBO via BR
> Core); `CodeSystem` local só é criado quando não há código padrão equivalente — ver `CLAUDE.md`.
